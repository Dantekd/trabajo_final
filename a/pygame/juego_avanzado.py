...
import pygame
import time
#ACA te da desorden en las letras importante cosa que en el principal no es asi
# ========== RUTAS ==========
...
def ejecutar_partida_pygame(partida: dict) -> int:
    ...
    while running:
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                while i < len(botones):
                    ...
                    if rect.collidepoint(pos):
                        # al clicar una letra, la usamos para revelar; no eliminamos el botón (visual opcional)
                        l = letra.upper()
                        encontrada = revelar_letra(l)
                        if encontrada:
                            mostrar_mensaje(f"Letra {l} encontrada")
                        else:
                            estado["errores"] = estado["errores"] + 1
                            mostrar_mensaje(f"Letra {l} incorrecta")
                        # también la agregamos a la palabra_armada para permitir luego enviar la palabra completa
                        palabra_actual = palabra_actual + l
                        # actualizar la variable en outer scope
            ...
            if ev.type == pygame.KEYDOWN:
                if ev.key == pygame.K_RETURN:
                    if len(palabra_actual) > 0:
                        # procesar palabra completa (usa la lógica del nucleo para puntaje/errores)
                        resultado = procesar_palabra_pygame(estado, palabra_actual)
                        if resultado == "valida":
                            mostrar_mensaje("Palabra válida")
                            # Si la palabra válida es exactamente la objetivo, rellenamos guiones
                    ...
                    # permitir ingreso de letras desde teclado para formar palabra_actual
                    keyname = ev.unicode
                    if keyname is not None and len(keyname) == 1 and keyname.isalpha():
                        palabra_actual = palabra_actual + keyname.upper()
      ...
      # comprobar objetivo completado
        ...
        # ---- NUEVO: comprobar si quedan palabras ----
